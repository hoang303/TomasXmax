...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n[🐣] => check ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n[🐣] => check del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n[🐣] => check onl => Xem thời gian hoạt động bot onl \n--------\n[🐣] => check tt => Check độ tương tác của bạn\n--------\n[🐣] => check box => Lọc nhóm dưới 4 thành viên\n--------\n[🐣] => check all => Check tất cả độ tương tác các thành viên trong box\n--------\n[🐣] => check covid => Xem thông tin covid\n--------\n[🐣] => check mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n[🐣] => check nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===
Điều hành bot bởi lvbang 🐷`, event.threadID, event.messageID);
    var arr = [];
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100028356152567) return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
