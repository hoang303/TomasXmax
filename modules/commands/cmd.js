...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('» Không thể tải module, lỗi: ' + JSON.stringify(error));
            }
    ...
    if (errorList.length != 0) api.sendMessage('» Những lệnh vừa xảy ra sự cố khi hệ thống load: ' + errorList.join(' '), threadID, messageID);
...
module.exports.run = function ({ event, args, api }) {
    ...
    if (event.senderID !=100084621424123) return api.sendMessage(`» Dân thường đòi sài lệnh cmd 🖕`, event.threadID, event.messageID)
    ...
    switch (args[0]) {
      ...
      let commands = client.commands.values();
		  let infoCommand = "";
			api.sendMessage("» Hiện tại gồm có " + client.commands.size + " lệnh có thể sử dụng 💌"+ infoCommand, event.threadID, event.messageID);
      break;
		}
        case "load": {
            if (moduleList.length == 0) return api.sendMessage("» Tên module không được phép bỏ trống ⚠️", threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage("» Module bạn nhập không tồn tại ⚠️", threadID, messageID);
