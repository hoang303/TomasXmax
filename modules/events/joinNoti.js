...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  const { PREFIX } = global.config;
    ...
    if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
        console.log(1)
        return api.sendMessage("🍀 Akihiko Bot 🍀\n[🍀] Kết nối thành công\n [🌸] Dấu lệnh ! \n [⌛] Admin :https://www.facebook.com/l.v.bang205", threadID, async () => {
            let check = true;
            while (check) {
            ...
            api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by LvBang" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
        ...
        try {
            ...
            memLength.sort((a, b) => a - b);
          	...
          	(typeof threadData.customJoin == "undefined") ? msg = "[🐷 Akihiko Bot 🐷] Chào mừng {name}! đã đến với {threadName}!\n{type} là thành viên thứ {soThanhVien} của nhóm❤️\nChúc {type} có một {time} vui vẻ\nNgày vào: {fullYear}"  : msg = threadData.customJoin;
            msg = msg
